...
try{
    ...
    if($action == "approve"){
        ...
        mysqli_query($conn,"UPDATE users SET role='organizer', status='active' WHERE id=$user_id");
    ...
    // Reset user role but keep account active (so can reapply)
    mysqli_query($conn,"
        UPDATE users 
        SET role='user',
            status='active'
    ...
    // Mark organizer as license cancelled
    mysqli_query($conn,"
        UPDATE organizers
    ...
    $log = "Organizer license cancelled (can reapply) (ID: $org_id)";
}
    ...
    $stmt = mysqli_prepare($conn,"
        ...
        VALUES(?,?)
    ");
    mysqli_stmt_bind_param($stmt,"is",$admin_id,$log);
